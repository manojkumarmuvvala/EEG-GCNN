...
def collect_metrics(y_probs_test, y_true_test, y_pred_test, sample_indices_test,
    ...
    dataset_index = pd.read_csv("D:\pythonProject\master_metadata_index.csv")
...
if __name__ == "__main__":
    ...
    MASTER_DATASET_INDEX = pd.read_csv("D:\pythonProject\master_metadata_index.csv",dtype= {'patient_ID' : 'str'})
    ...
    heldout_test_indices = MASTER_DATASET_INDEX.index[
        MASTER_DATASET_INDEX["patient_ID"].astype("str").isin(test_subjects)].tolist()
    ...
    X = load("D:\pythonProject\psd_features_data_X")
    y = load("D:\pythonProject\spec_coh_values.npy")
    ...
    for FOLD_IDX in range(10):
        ...
        auroc_patient_test, auroc_test, precision_patient_test, recall_patient_test, f1_patient_test, bal_acc_patient_test = collect_metrics(
    ...
    print(
        f"10-folds-avg heldout test patient AUROC: {stats.mean(auroc_patient_test_folds)} ({stats.stdev(auroc_patient_test_folds)})")
    print(
        f"10-folds-avg heldout test patient PRECISION: {stats.mean(precision_patient_test_folds)} ({stats.stdev(precision_patient_test_folds)})")
    print(
        f"10-folds-avg heldout test patient RECALL: {stats.mean(recall_patient_test_folds)} ({stats.stdev(recall_patient_test_folds)})")
    print(
        f"10-folds-avg heldout test patient F-1: {stats.mean(f1_patient_test_folds)} ({stats.stdev(f1_patient_test_folds)})")
    print(
        f"10-folds-avg heldout test patient BALANCED ACCURACY: {stats.mean(bal_acc_patient_test_folds)} ({stats.stdev(bal_acc_patient_test_folds)})")

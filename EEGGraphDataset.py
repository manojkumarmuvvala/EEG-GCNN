...
class EEGGraphDataset(Dataset):
    def __init__(self, X, y, indices, loader_type, sfreq, transform=None):
        ...
        self.spec_coh_values = np.load("D:\pythonProject\spec_coh_values.npy", allow_pickle=True)

    # sensor distances don't depend on window ID
    def get_sensor_distances(self):
        import pandas as pd
        coords_1010 = pd.read_csv("D:\pythonProject\standard_1010.tsv.txt", sep='\t')
    ...
    def get_geodesic_distance(self, montage_sensor1_idx, montage_sensor2_idx, coords_1010):
        ...
        x1 = float(coords_1010[coords_1010.label == ref_sensor1]["x"].iloc[0])
        y1 = float(coords_1010[coords_1010.label == ref_sensor1]["y"].iloc[0])
        z1 = float(coords_1010[coords_1010.label == ref_sensor1]["z"].iloc[0])

        # print(ref_sensor2, montage_sensor2_idx, coords_1010[coords_1010.label == ref_sensor2]["x"])
        x2 = float(coords_1010[coords_1010.label == ref_sensor2]["x"].iloc[0])
        y2 = float(coords_1010[coords_1010.label == ref_sensor2]["y"].iloc[0])
        z2 = float(coords_1010[coords_1010.label == ref_sensor2]["z"].iloc[0])
